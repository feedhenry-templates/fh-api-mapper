...
var headersToRequestFormat = function (headers) {
  var newHeaders = {};
...
function checkCache(requestObject, cb) {
  if (requestObject.cache > 0) {
    var options = {
      "act": "load",
      "key": requestObject._id
    };
...
module.exports = function (requestObject, debug, cb) {
  ...
  checkCache(requestObject, function (err, cachedReponse) {
    if(cachedReponse){
       cb(null, {
        request: {
          headers: headers,
    ...
    r = request[method]({
      ...
      if (requestObject.cache > 0) {
        var options = {
          "act": "save",
          "key": requestObject._id, // The key associated with the object
          "value": responseObj, // The value to be cached, must be serializable
          "expire": requestObject.cache  // Expiry time in seconds. Optional
        };
        $fh.cache(options, function () {
        });
      }
      return cb(null, {
        request: {
